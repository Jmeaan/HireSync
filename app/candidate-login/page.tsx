...
export default function CandidateLoginPage() {
  const { login, authError, clearAuthError } = useATSStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (result.success) {
      router.replace(result.role === 'manager' ? '/manager/dashboard' : '/candidate/dashboard');
    }
  };
        ...
        <div className="hs-fade-in" style={{ width: '100%', maxWidth: 440 }}>
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <p style={{ fontSize: 16, color: '#94a3b8', marginTop: 12, lineHeight: 1.6 }}>
              Sign in to track your applications and stay updated on your status.
            </p>
          </div>
          ...
          {/* Footer links */}
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: `1px solid ${BORDER}`, display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
            <p style={{ fontSize: 15, color: '#94a3b8', margin: 0 }}>
              No account?{' '}
              <Link href="/candidate-register" style={{ color: '#a78bfa', fontWeight: 600, textDecoration: 'none', transition: 'color 0.15s' }}
                onMouseEnter={(e) => (e.currentTarget.style.color = '#fff')}
